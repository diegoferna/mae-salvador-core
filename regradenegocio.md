1. Modelagem de dados (base atual)
   ...
   ESUS_DATABASE_URL: apoio de geolocalização e função de orientação integrada ao e-SUS.
   ...
   Principais campos: gestante_id FK, ubs_id FK, unidade_escolhida_nome, unidade_escolhida_origem (cdi|proxima)
   ...
   apoio_cnes_gps
   no_unidade PK, latitude/longitude da unidade
   apoio_paciente_gps
   nu_cns PK, latitude/longitude do paciente, atualizado_em
   Funções SQL:
   fn_orientacao_mae_salvador(cns, lat, lon)
   fn_orientacao_mae_salvador(cns) (overload legado/compatibilidade)
...
2. Regras de negócio
   ...
   Coordenadas geográficas, quando fornecidas, precisam estar em faixa válida.
   Escopo de papel:
...
3. Fluxos principais do sistema
   ...
   Entrada: cns ou cadastroId, coordenadas opcionais.
   Processamento:
   resolve CNS;
   tenta geocodificação por endereço;
   grava GPS de paciente no e-SUS;
   executa fn_orientacao_mae_salvador.
   Saída: mensagem final de orientação.
   Escolha de unidade
   Entrada: cadastroId, nome da unidade, origem da escolha.
   Processamento: resolve UBS, atualiza vínculo e histórico.
...
5. Dependências e integrações
   ...
   PostgreSQL (e-SUS): CEP, apoio geográfico e orientação.
   ...
   lógica de confirmação depende da função SQL do e-SUS;
...
7. Pontos críticos do sistema (não quebrar)
   ...
   Fluxo de confirmação que depende da função fn_orientacao_mae_salvador.
...
8. Diretrizes de reimplementação (NestJS) mantendo o banco
   ...
   Isolar integrações externas por adaptadores (e-SUS PG, SOAP CadSUS, Nominatim), com timeout/circuit-breaker.
